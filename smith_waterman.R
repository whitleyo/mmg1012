...
smith_wat <- function(seq1, #character vector of length 1
  ...
  for (p in 1:nrow(end_pts)) {
    ...
    while(cont) {
      ...
      base2_ind <- which(base_set == base2)
      # Find H[i-1,j-1] + s(ai,bj)
      option1 <- H[i-1,j-1] + BLOSUM[base1_ind, base2_ind] # if max, then at i and j, perfect alignment
    ...
    solutions[[p]] <- list( seq1_ali = out_seq1, alignment = ali_seq, seq2_ali = out_seq2, score = max(H))
  } # end larger for loop
  return(solutions)
}
